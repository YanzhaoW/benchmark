...
namespace
{
    ...
    void TestDenseMatrix(benchmark::State& state)
    {
        ...
        auto matrix = Eigen::MatrixXf::Zero(n_rows_min, n_cols_min).eval();
    ...
    void TestConResize(benchmark::State& state)
    {
        ...
        for (auto idx : state)
        {
            ...
            matrix.conservativeResize(n_row, n_col);
            dense_randomize(matrix, engine);
            sum += matrix.sum() / (n_row * n_col);
        }
        if constexpr (enable_hist)
    ...
    void TestVector(benchmark::State& state)
    {
        ...
        vec.resize(n_rows_min * n_cols_min, 0.F);
...
BENCHMARK(TestConResize)->Threads(1)->Iterations(100000)->Name("Using dense matrix conservative resize");
